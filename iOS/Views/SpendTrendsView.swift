    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .top) {
                ...
                // Fades scrolled content out before it reaches the nav bar —
                // done by hand rather than via scrollEdgeEffectStyle, which
                // turned out not to render reliably in TestFlight builds
                // (works when Xcode installs the app straight to a device,
                // not once it's gone through Xcode Cloud/App Store
                // distribution, even with the same Xcode version pinned).
                // A plain gradient has no dependency on that system behavior.
                topFadeOverlay
            }
